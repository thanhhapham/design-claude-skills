...
Exception: passive voice is OK in notifications and error messages where the outcome matters more than the actor — "Your card won't be charged now", "Account suspended", "Promo code applied".
...
Don't contract the main verb ("Contact us if you've any questions" is wrong) or a noun + verb ("The item'll be delivered" is wrong). When you need to stress a negative, use "do not" instead of "don't" — it reads stronger ("Do not leave the page").

### 4. Plain language, always
No jargon, no internal product naming that leaks into user-facing copy, no legalese. If a word needs explanation, replace it. No slang — Carousell's audience ranges from teens to elderly, so slang excludes readers.

### 5. Simple past over present perfect
Prefer "You reached your limit" over "You've reached your limit" — unless the time factor genuinely matters (e.g. "You've been a member since 2020").

### 6. Inclusive pronouns
Use "they/them" for gender-neutral references, not "he/she" or "him/her".
...
Use sparingly. Exclamation marks only for genuine celebration moments ("Your item sold!"). Never use them for routine states. Never use multiple ("!!!" is never acceptable). Never use in serious contexts (suspensions, scams, COVID-19 messaging, safety issues). Max one exclamation mark per screen elsewhere.

### "And" not "&"
Spell out "and" in copy. Exception: brand/category names where "&" is part of the proper noun ("Health & Beauty").

### Quotation marks
Use quotation marks to signal exact product text — quote the actual button, label, or menu item: "Tap 'List' to view your drafts". Single quotes by default ('like this'); double quotes only when quoting within a quote ("he said 'like this'").

### Links
Embed links in a verb+noun phrase, never show a raw URL: "Share your thoughts with us" not "Share your thoughts at https://...".

### Slashes and promo codes
No spaces around slashes: "Bump/Spotlight", not "Bump / Spotlight". Promo codes go in angle brackets and CAPS: "Use promo code <SHIPTOME> for $5 off".
...
- Zero: spell out ("no listings yet") unless it refers to money ("$0")
...
| Listing form | Listing page / Post a listing page / Sell form |
...
| Mobile number | Handphone / Phone number |

**Meet-up vs meet up:** "Meet-up" (noun) is the deal method ("Choose meet-up as your delivery method"). "Meet up" (verb) is the action ("Meet up with the buyer this weekend").

---

## Professional Seller Rules

When copy mentions "Professional seller", "Personal seller", or account types:

- Always capitalize "Professional" and "Personal" — they're proper nouns ("You're on a Professional account", "Professional seller badge")
- Never say "professional account seller" — use either "Professional account" or "Professional seller", never both
- Never say "Professional badge" — the badge itself reads "Professional seller", so quote it as such

---

## Directing Users

- Don't call it a "button" — just name the action: "Tap 'Set up contract'" (obvious enough it's tappable)
- Don't use colour to direct users (some are colourblind) — say "Tap 'Request for contract' in Chat", not "Tap the green button"
